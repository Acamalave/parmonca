    ...
    <div className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-0.5">{clientes.length} registros</p>
        </div>
          ...
          <Link key={cliente.id} href={`/clientes/${cliente.id}`}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-medium text-[var(--color-text-secondary)] truncate text-[13px] group-hover:text-[#E8821C] transition-colors">{cliente.nombre}</h3>
                <p className="text-[11px] text-[var(--color-text-muted)] mt-0.5">{cliente.contactoPrincipal} · {cliente.cargoContacto}</p>
              </div>
            ...
            <div className="mt-3 space-y-1.5">
              <div className="flex items-center gap-2 text-[11px] text-[var(--color-text-secondary)]"><MapPin size={11} /><span>{cliente.ciudad}, {cliente.pais}</span></div>
              <div className="flex items-center gap-2 text-[11px] text-[var(--color-text-secondary)]"><Mail size={11} /><span className="truncate">{cliente.email}</span></div>
              <div className="flex items-center gap-2 text-[11px] text-[var(--color-text-secondary)]"><Phone size={11} /><span>{cliente.telefono}</span></div>
            </div>
            <div className="mt-3 pt-2.5 border-t border-[var(--color-border)] flex items-center justify-between">
              <span className="text-[10px] text-[var(--color-text-muted)]">{cliente.comercialAsignado}</span>
              <span className="text-[10px] text-[var(--color-text-muted)]">{cliente.empresaAsignada}</span>
            </div>
          </Link>
        ))}
      </div>
