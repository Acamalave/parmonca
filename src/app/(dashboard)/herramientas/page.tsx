...
export default function HerramientasPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      <div>
        <h1 className="font-display text-2xl font-bold text-white tracking-tight">Herramientas</h1>
        <p className="text-sm text-zinc-600 mt-0.5">Espacio de trabajo del colaborador</p>
      </div>
        ...
        {acciones.map((a) => {
          ...
          return (
            <Link key={a.label} href={a.href}
              className="glass rounded-xl p-4 hover:bg-white/[0.04] transition-all group text-center">
              ...
              <p className="text-[13px] font-semibold text-zinc-200">{a.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <div className="glass rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[13px] font-semibold text-zinc-300">Mis Pendientes</h3>
          ...
          <div className="space-y-2">
            {cotizacionesPendientes.slice(0, 3).map((cot) => (
              <Link key={cot.id} href={`/cotizaciones/${cot.id}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-medium text-zinc-300 truncate">{cot.clienteNombre}</p>
                  <p className="text-[10px] text-zinc-600">{cot.numero} · <span className="font-num">{formatCurrency(cot.total)}</span></p>
                </div>
            ...
            {leadsSinAtender.slice(0, 2).map((lead) => (
              <Link key={lead.id} href={`/clientes/${lead.id}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-medium text-zinc-300 truncate">{lead.nombre}</p>
                  <p className="text-[10px] text-zinc-600">Lead · {lead.pais}</p>
                </div>
            ...
            {facturasVencidas.slice(0, 2).map((fac) => (
              <Link key={fac.id} href={`/facturas/${fac.id}`}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-medium text-zinc-300 truncate">{fac.clienteNombre}</p>
                  <p className="text-[10px] text-zinc-600">{fac.numero} · <span className="font-num">{formatCurrency(fac.total)}</span></p>
                </div>
        ...
        <div className="space-y-3">
          <div className="glass rounded-xl p-4">
            <h3 className="text-[13px] font-semibold text-zinc-300 mb-3">Mi Rendimiento</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center">
                <p className="text-xl font-num font-bold text-white">{misCotizaciones.length}</p>
                <p className="text-[10px] text-zinc-600 mt-0.5">Cotizaciones</p>
              </div>
              <div className="text-center">
                <p className="text-xl font-num font-bold text-emerald-400">{misCotizaciones.filter(c => c.estado === 'aceptada').length}</p>
                <p className="text-[10px] text-zinc-600 mt-0.5">Ganadas</p>
              </div>
              <div className="text-center">
                ...
                <p className="text-[10px] text-zinc-600 mt-0.5">Vendido</p>
              </div>
            </div>
          </div>
          ...
          {/* Recent Activity */}
          <div className="glass rounded-xl p-4">
            <h3 className="text-[13px] font-semibold text-zinc-300 mb-3">Actividad Reciente</h3>
              ...
              {[
                ...
                return (
                  <div key={i} className="flex items-center gap-2.5">
                    ...
                    <p className="text-[12px] text-zinc-400 flex-1">{item.text}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div className="glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-[13px] font-semibold text-zinc-300">Agenda de Hoy</h3>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1">
                  <p className="text-[12px] text-zinc-300">{ev.titulo}</p>
                  <p className="text-[10px] text-zinc-600">{ev.tipo}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="glass rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-[13px] font-semibold text-zinc-300">Metas del Mes</h3>
          </div>
          <div className="space-y-3">
            {[
              ...
              <div key={m.meta}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[12px] text-zinc-400">{m.meta}</span>
                  <span className="text-[12px] font-num font-medium text-zinc-300">{m.actual}/{m.objetivo}</span>
                </div>
